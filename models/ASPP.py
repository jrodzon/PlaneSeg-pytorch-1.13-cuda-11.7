...
class ASPP_FPN(nn.Module):
    def __init__(self, C1, C2, C3, C4, C5, out_channels, bilinear_upsampling=False):
        ...
        self.aspp = ASPP() # NOTE! if you use ResNet50-152, set self.aspp = ASPP_Bottleneck(num_classes=self.num_classes) instead
    ...
    def forward(self, x):
        ...
        context = self.aspp(c5) # (shape: (batch_size, 256, h/16, w/16))
        #output = F.upsample(output, size=(h, w), mode="bilinear") # (shape: (batch_size, 256, h, w))

        return c2, c3, c4, context


#--aspp--#

class MaskRCNN_ASPP(nn.Module):
    ...
    def __init__(self, config, model_dir='test'):
        ...
        """
        super(MaskRCNN_ASPP, self).__init__()
        self.config = config
    ...
    def build(self, config):
        ...
        self.fpn = ASPP_FPN(C1, C2, C3, C4, C5, out_channels=256)
    ...
    def predict(self, input, mode, use_nms=1, use_refinement=False, return_feature_map=False):
        ...
        c2, c3, c4, context = self.fpn(molded_images) # (context shape: (batch_size, 256, h/16, w/16))
        edge, edge_fea = self.edge_layer(c2, c3, c4)

        context_up = F.upsample(context, scale_factor=8, mode="bilinear")
        h, w = context.shape[2:]
        edge_fea_up = []
        for f in edge_fea:
            edge_fea_up.append(F.interpolate(f, size=(h*8, w*8), mode='bilinear', align_corners=True))

        p2_out = torch.cat([*edge_fea_up, context_up], dim=1)
        ...
        # p2_out shape: (batch_size, 256, h/4, w/4)
        # p3_out shape: (batch_size, 256, h/8, w/8)
        # p4_out shape: (batch_size, 256, h/16, w/16)
        # p5_out shape: (batch_size, 256, h/32, w/32)
        # p6_out shape: (batch_size, 256, h/64, w/64)
        # edge   shape: (batch_size, 2, h/4, w/4)
