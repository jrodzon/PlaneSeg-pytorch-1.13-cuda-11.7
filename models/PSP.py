...
class MaskRCNN_PSP(nn.Module):
    ...
    def __init__(self, config, model_dir='test'):
        ...
        """
        super(MaskRCNN_PSP, self).__init__()
        self.config = config
    ...
    def predict(self, input, mode, use_nms=1, use_refinement=False, return_feature_map=False):
        ...
        upscale_edge_fea = []
        for f in edge_fea:
            upscale_edge_fea.append(F.interpolate(f, size=context.shape[2:], mode='bilinear', align_corners=True))
        p2_out = torch.cat([*upscale_edge_fea, context], dim=1)
