...
def evaluate(options):
    ...
    for name, detector in detectors:
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            ...
            if sampleIndex < 300 or options.debug or options.dataset != '':
                visualizeBatchPair(options, config, input_pair, detection_pair, indexOffset=sampleIndex, suffix='_' + name + options.modelType, write_ply=False, write_new_view=options.testingIndex >= 0 and 'occlusion' in options.suffix)
                pass
            if sampleIndex >= options.numTestingImages:
                break
            continue
        if 'inference' not in options.dataset:
            ...
            printStatisticsDetection(options, statistics, note='_ablation_vis_3d_pr')
    ...
    if 'inference' not in options.dataset:
        ...
        if options.testingIndex == -1:
            npyName = 'logs/all_statistics%s.npy' % '_ablation_vis_3d_pr'
