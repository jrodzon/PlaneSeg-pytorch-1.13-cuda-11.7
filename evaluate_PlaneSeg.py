...
class PlaneRCNNDetector():
    def __init__(self, options, config, modelType, checkpoint_dir=''):
        ...
        # self.model = MaskRCNN_edge_fpn_resolution_v2(config)
        # self.model = MaskRCNN(config)
        # self.model = MaskRCNN_base(config)
        self.model = MaskRCNN_edge_fpn_resolution(config)
        # self.model = MaskRCNN_paper(config)
        
        # self.model = MaskRCNN_res_concat(config)
        self.model.cuda()
        self.model.eval()
        ...
        checkpoint_dir="checkpoint/planercnn_normal_warping_refine_ablation_edge_fpn_resolution_edge_rgb_ablation_edge_fpn_resolution"
    ...
    def detect(self, sample):
        ...
        if ('refine' in self.modelType or 'refine' in self.options.suffix):
            ...
            for c in range(1):
                ...
                continue
            pass
        return detection_pair, gt_depth
...
def evaluate(options):
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            ...
            if 'inference' not in options.dataset:
                for c in range(len(input_pair)):
                    my_statistics=evaluateBatchDetection(options, config, input_pair[c], detection_pair[c], statistics=statistics, printInfo=options.debug, evaluate_plane=options.dataset == '')
                    # all_statistics.append(statistics)
                    depth_statistics.append(my_statistics[1])
                    continue
                # print(statistics)
                # print(np.array(statistics).shape)
                # rvs_array = np.array([np.array(x) for x in statistics[2]])
                # print(rvs_array.shape)
                # depth_statistics=my_statistics[1]
    ...
    if 'inference' not in options.dataset:
        ...
        if options.testingIndex == -1:
            npyName = 'logs/all_statistics%s.npy' % '_res_concat'
            np.save(npyName, all_statistics)
            pass
        pass
    ...
    rvs_array = np.array([np.array(x) for x in all_statistics[0][2]])
    aps_array = np.array([np.array(x) for x in all_statistics[0][3]])
    print(rvs_array.shape)
    print(aps_array.shape)
    print('========================================')
    print('RI', np.mean(rvs_array[:,0]))
    print('VOI', np.mean(rvs_array[:,1]))
    print('SC', np.mean(rvs_array[:,2]))
    # rel rmse 
    print("depth_stastics:",my_statistics[1])

    
    print('AP03', np.mean(aps_array[:,0]))
    print('AP06', np.mean(aps_array[:,1]))
    print('AP09', np.mean(aps_array[:,2]))
